...
#if defined(NDEBUG)
# error "Luckyblockcoin cannot be compiled without assertions."
#endif
...
const std::string strMessageMagic = "Luckyblockcoin Signed Message:\n";
                              ...
                              bool fOverrideMempoolLimit, const CAmount& nAbsurdFee, std::vector<uint256>& vHashTxnToUncache)
{
...

    {
        ...
        {
            LogPrint("mempool", "replacing tx %s with %s for %s LUB additional fees, %d delta bytes\n",
...
void ThreadScriptCheck() {
    RenameThread("luckyblockcoin-scriptch");
