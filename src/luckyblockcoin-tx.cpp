...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc<2 || IsArgSet("-?") || IsArgSet("-h") || IsArgSet("-help"))
    {
        ...
        std::string strUsage = strprintf(_("%s luckyblockcoin-tx utility version"), _(PACKAGE_NAME)) + " " + FormatFullVersion() + "\n\n" +
            _("Usage:") + "\n" +
              "  luckyblockcoin-tx [options] <hex-tx> [commands]  " + _("Update hex-encoded luckyblockcoin transaction") + "\n" +
              "  luckyblockcoin-tx [options] -create [commands]   " + _("Create hex-encoded luckyblockcoin transaction") + "\n" +
              "\n";
...
static void MutateTxAddOutAddr(CMutableTransaction& tx, const std::string& strInput)
{
    ...
    CLuckyblockcoinAddress addr(strAddr);
    if (!addr.IsValid())
...
static void MutateTxAddOutPubKey(CMutableTransaction& tx, const std::string& strInput)
{
    ...
    CLuckyblockcoinAddress addr(scriptPubKey);
    ...
    if (bScriptHash) {
        ...
        CLuckyblockcoinAddress redeemScriptAddr(scriptPubKey);
...
static int CommandLineRawTx(int argc, char* argv[])
{
    ...
    try {
        ...
        if (!fCreateBlank) {
            ...
            // param: hex-encoded luckyblockcoin transaction
