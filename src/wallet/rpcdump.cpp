...
UniValue importprivkey(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 3)
        throw runtime_error(
            "importprivkey \"luckyblockcoinprivkey\" ( \"label\" ) ( rescan )\n"
            "\nAdds a private key (as returned by dumpprivkey) to your wallet.\n"
            "\nArguments:\n"
            "1. \"luckyblockcoinprivkey\"   (string, required) The private key (see dumpprivkey)\n"
    ...
    CLuckyblockcoinSecret vchSecret;
...
void ImportAddress(const CLuckyblockcoinAddress& address, const string& strLabel);
void ImportScript(const CScript& script, const string& strLabel, bool isRedeemScript)
{
    ...
    if (isRedeemScript) {
        ...
        ImportAddress(CLuckyblockcoinAddress(CScriptID(script)), strLabel);
    } else {
...
UniValue importaddress(const JSONRPCRequest& request)
{
    ...
    if (address.IsValid()) {
        ...
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid Luckyblockcoin address or script");
    }
...
UniValue importpubkey(const JSONRPCRequest& request)
{
    ...
    ImportAddress(CLuckyblockcoinAddress(pubKey.GetID()), strLabel);
...
UniValue importwallet(const JSONRPCRequest& request)
{
    ...
    while (file.good()) {
        ...
        if (pwalletMain->HaveKey(keyid)) {
            LogPrintf("Skipping import of %s (key already present)\n", CLuckyblockcoinAddress(keyid).ToString());
            continue;
        }
...
UniValue dumpprivkey(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 1)
        throw runtime_error(
            ...
            "\nArguments:\n"
            "1. \"address\"   (string, required) The luckyblockcoin address for the private key\n"
            "\nResult:\n"
    ...
    CLuckyblockcoinAddress address;
    ...
    return CLuckyblockcoinSecret(vchSecret).ToString();
}
...
UniValue dumpwallet(const JSONRPCRequest& request)
{
    ...
    // produce output
    file << strprintf("# Wallet dump created by Luckyblockcoin %s\n", CLIENT_BUILD);
    ...
    if (!masterKeyID.IsNull())
    {
        ...
        if (pwalletMain->GetKey(masterKeyID, key))
        {
            ...
            CLuckyblockcoinExtKey b58extkey;
    ...
    for (std::vector<std::pair<int64_t, CKeyID> >::const_iterator it = vKeyBirth.begin(); it != vKeyBirth.end(); it++) {
        ...
        std::string strAddr = CLuckyblockcoinAddress(keyid).ToString();
        CKey key;
        if (pwalletMain->GetKey(keyid, key)) {
            file << strprintf("%s %s ", CLuckyblockcoinSecret(key).ToString(), strTime);
...
UniValue ProcessImport(const UniValue& data, const int64_t timestamp)
{
    try {
        ...
        if (!isScript) {
            address = CLuckyblockcoinAddress(output);
            if (!address.IsValid()) {
        ...
        if (isP2SH) {
            ...
            CLuckyblockcoinAddress redeemAddress = CLuckyblockcoinAddress(CScriptID(redeemScript));
            ...
            if (pubKeys.size() && keys.size() == 0) {
                ...
                CLuckyblockcoinAddress pubKeyAddress = CLuckyblockcoinAddress(pubKey.GetID());
                ...
                // Consistency check.
                if (isScript) {
                    CLuckyblockcoinAddress scriptAddress;
                    CTxDestination destination;

                    if (ExtractDestination(script, destination)) {
                        scriptAddress = CLuckyblockcoinAddress(destination);
