...
###   This script attempts to download the signature file SHA256SUMS.asc from luckyblockcoin.org
...
WORKINGDIR="/tmp/luckyblockcoin_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
BASEDIR="https://luckyblockcoin.org/bin/"
VERSIONPREFIX="luckyblockcoin-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'luckyblockcoin-',
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Luckyblockcoin Core binary release signing key installed?"
   fi
