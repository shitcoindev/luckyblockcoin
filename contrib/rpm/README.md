...
The RPM spec file provided here is for Luckyblockcoin-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://luckyblockcoin.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Luckyblockcoin git repository](https://github.com/luckyblockcoin/luckyblockcoin) but are not part of
...
Currently only `Source10` is used, for the example `luckyblockcoin.conf` file.
...
in the luckyblockcoin git repository. At present only an SVG version of the Luckyblockcoin
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the luckyblockcoin
...
needed when 0.12.1 is released, a proper fix is already in the Luckyblockcoin git
master branch.
...
idea to build Luckyblockcoin Core against a static version of that library compiled
...
The `luckyblockcoin.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `luckyblockcoin-server` package. At this point, most RPM based
...
The `luckyblockcoin-server` package should have SELinux support. How to properly do
...
Luckyblockcoin-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/luckyblockcoin/). He has been
packaging Luckyblockcoin for Fedora at least since 2012.
...
4. I renamed the `luckyblockcoin` package that contains the Qt GUI to `luckyblockcoin-core` as
that appears to be how the general population refers to it, in contrast to
`luckyblockcoin-xt` or `luckyblockcoin-classic`. I wanted to make sure the general population
